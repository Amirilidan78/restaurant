          ...
          <thead>
            ...
            <tr>
              <th class="search-item head-th" v-for="item of _searches">
                ...
                <input
                  v-else-if="item.input === 'text'" type="text"
                  class="form-control form-control-solid form-control-sm text-start"
...
export default {
  ...
  computed : {
    from : function (){
      ...
      if ( !!this.per_page && !!this.current_page )
  ...
  methods : {
    ...
    fetchMethod(){
      ...
      .then( result => {
        ...
        const { current_page ,total ,last_page ,per_page ,to ,links } = meta ;
        let pages = [] ;
    ...
    // getItemSortPrefix( item ){
    //   if( item )
    //   {
    //     if( this.sort.key === item.key )
    //     {
    //       if( item.type === 'relation-equal' )
    //       {
